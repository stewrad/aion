...
class DVB_S2_Receiver:
    ...
    def initialize(self):
        ...
        self.mcs_walsh_table = mcs_walsh_gen(32)  # 32 MCS options
    ...
    def identify_mcs(self, mcs_symbols: np.ndarray) -> Tuple[int, float]:
        """
        Identify MCS from Walsh-coded header symbols
        ...
        for idx in range(32):
            ...
            # Correlate received symbols with each Walsh code
            corr = np.abs(np.vdot(mcs_symbols[:len(walsh_sym)], walsh_sym))
            corr = corr / (np.linalg.norm(walsh_sym) * np.linalg.norm(mcs_symbols[:len(walsh_sym)]))
    ...
    def process_frame(self, frame_symbols: np.ndarray) -> Optional[FrameMetrics]:
        ...
        mcs_len = len(self.mcs_walsh_table)  # Assuming this is the symbol length
        mcs_symbols = frame_symbols[frame_start:frame_start + mcs_len]
        ...
        data_start = frame_start + mcs_len
    ...
    def receive_zmq_stream(self, timeout_ms: int = 5000):
        """Receive and process ZMQ stream"""
        
        logger.info("Waiting for ZMQ stream...")
        
        # Receive metadata first
        if self.rx_socket.poll(timeout_ms):
            metadata_str = self.rx_socket.recv_string()
        ...
        while True:
            try:
                if self.rx_socket.poll(timeout_ms):
                    ...
                    all_symbols.append(chunk)
                    chunk_count += 1
                    
                    if chunk_count % 10 == 0:
                        logger.info(f"Received {chunk_count} chunks, "
                                   f"{len(np.concatenate(all_symbols))} symbols")
                else:
                    # Timeout - assume end of transmission
        ...
        # Concatenate all received symbols
        received_signal = np.concatenate(all_symbols)
        logger.info(f"Total received: {len(received_signal)} symbols")
    ...
    def run(self, frame_length: int = 20000):
        """Main receiver loop"""
        ...
        try:
            ...
            # Receive stream
            received_signal = self.receive_zmq_stream(timeout_ms=5000)
            
            if received_signal is not None:
                # Process all frames in the signal
...
def main():
    ...
    receiver.run(frame_length=args.frame_length)
