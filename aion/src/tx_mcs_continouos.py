...
def build_burst(snr, meta, sof, mcs_walsh, current_mcs_idx, pilots):
    """
    ...
    mcs_name = list(MCS_LOOKUP.keys())[current_mcs_idx] if current_mcs_idx in range(len(MCS_LOOKUP)) else select_mcs_for_snr(snr)
...
# Burst Thread
# --------------------------
def burst_thread(socket, meta, sof, mcs_walsh, pilots, current_mcs_idx, burst_active):
    ...
    while True:
        if burst_active.is_set():
            ...
            burst = build_burst(snr, meta, sof, mcs_walsh, current_mcs_idx, pilots)
            socket.send(burst.tobytes())
...
def csi_listener(csi_socket, current_mcs_idx):
    """Listen for CSI feedback and update global MCS index"""
    import json
    while True:
        try:
            msg = csi_socket.recv(flags=zmq.NOBLOCK)
            feedback = json.loads(msg.decode())
            mcs_idx = feedback.get("recommended_mcs_idx", 0)
            snr_db = feedback.get("snr_db", 0)
            print(f"[CSI] RX reports SNR={snr_db:.1f} dB → MCS {mcs_idx}")
            current_mcs_idx = mcs_idx
        except zmq.Again:
            time.sleep(0.05)
...
def main():
    ...
    # Feedback subscriber (from RX)
    csi_socket = ctx.socket(zmq.SUB)
    csi_socket.connect("tcp://127.0.0.1:5556")
    csi_socket.setsockopt_string(zmq.SUBSCRIBE, "")
    logging.info(f"[TX] Connected to CSI feedback at tcp://127.0.0.1:5556")

    # Shared variable for adaptive MCS
    current_mcs_idx = 0
    ...
    t_csi = threading.Thread(target=csi_listener, args=(csi_socket, current_mcs_idx), daemon=True)
    t_csi.start()

    t_noise = threading.Thread(target=noise_thread, args=(socket, Fs, noise_level), daemon=True)
    t_burst = threading.Thread(target=burst_thread, args=(socket, meta, sof, mcs_walsh, pilots, current_mcs_idx, burst_active), daemon=True)
    t_noise.start()
    t_burst.start()
